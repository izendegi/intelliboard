...
$cids = optional_param('cids', 0, PARAM_INT);
...
$PAGE->set_url(new moodle_url("/local/intelliboard/pf/activities.php", array("id"=>$id, "cids" => $cids, "search"=>$search, "status"=>$status)));
$PAGE->set_pagetype('activities');
...
$table = new intelliboard_pf_activities_table('table', $search, $id, $cohort->id, $status, $cids);
	...
	<div class="intelliboard-pf-content pf-table">
		<div class="intelliboard-search clearfix">
			...
			<form action="" method="GET" id="filterform">
				<select id="cids" class="pull-left cids singleselect coursesdropdown form-control" style="margin-right:20px; width: 200px;" multiple="multiple">
...
<script type="text/javascript">
	jQuery(document).ready(function(){
		...
		$("#fields  .ms-drop").on('click', 'button', function(){
			...
			location = "<?php echo new moodle_url("/local/intelliboard/pf/activities.php"); ?>?id="+id;
		});
		...
		$("#filterform  .ms-drop").on('click', 'button', function(){
			var cids = jQuery('#cids').val();
			var id = jQuery('#fieldid').val();
			location = "<?php echo new moodle_url("/local/intelliboard/pf/activities.php"); ?>?id="+id + "&cids="+cids;
		});
		...
		if (cids == '' && redirect == 0) {
			...
			var cids = jQuery('#cids').val();
			var id = jQuery('#fieldid').val();
			location = "<?php echo new moodle_url("/local/intelliboard/pf/activities.php"); ?>?id="+id + "&cids="+cids+"&redirect=1";
		}

	});
</script>
<style>
.singleselect input[type="checkbox"] {
	display: none;
}
.singleselect input[type="radio"] {
	margin-right: 5px;
	float: none;
}
</style>
